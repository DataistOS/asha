# پالت رنگی (Colors)

این رنگ‌ها «منبع حقیقت» (SSOT) برای تمامی محصولات آشا هستند.

```{note}
مقادیر زیر مستقیماً از فایل `tokens/color-palette.json` استخراج شده‌اند.

```

## بخش دوم: ساختار گرید

```{grid}
:gutter: 3

```{grid-item}
:columns: 6
```{grid-item-card} Primary
:shadow: md
**#7aa2f7**
رنگ اصلی برند و اکشن‌های کلیدی.
```

```
```{grid-item}
:columns: 6
```{grid-item-card} Secondary
:shadow: md
**#bb9af7**
رنگ ثانویه برای تاکیدات بصری.
```

## بخش سوم: جدول جزئیات رنگ‌ها

| نام توکن          | مقدار HEX | پیش‌نمایش |
|-------------------|-----------|-----------|
| `color-bg-main`   | `#1a1b26` | `#1a1b26` |
| `color-text-main` | `#a9b1d6` | `#a9b1d6` |
| `color-primary`   | `#7aa2f7` | `#7aa2f7` |
| `color-accent`    | `#ff9e64` | `#ff9e64` |
| `color-success`   | `#73daca` | `#73daca` |

